...
public class H_Select_topinfo_conn {

	static H_Select_topinfo_conn mainConn = new H_Select_topinfo_conn();
	// 아래 매소드에서 실제 명령문이 적혀이는 xml에서  받아온 값을 가지고 다른파일에 가지위해서 싱글톤형식으로 객체화
	public static H_Select_topinfo_conn instance() {
		return mainConn;
	}
	...
	public List<H_getset> h_select() {
	  ...
	  //리턴타입 확인 후 객체화- openSession = 데이터베이스 명령문을 그대로 집어넣는게 아닌 클래스를 이용하기위해
	    List<H_getset> list = ret_dbconn.selectList("select_top10_info"); 
